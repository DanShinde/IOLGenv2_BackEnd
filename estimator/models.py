...
class ProjectModule(models.Model):
    ...
    from_import = models.BooleanField(
        default=False,
        help_text="Set when this row was (re)generated by consolidating the project's ModuleImportRow rows "
                   "(see project_import_review's 'Confirm & Next'). Such rows are fully replaced every "
                   "time that consolidation runs again -- edit the underlying import rows on the Review & "
                   "Correct step for line-level corrections that need to survive it. A row added directly "
                   "here (from_import=False) is never touched by that process.",
    )
...
class ModuleImportRow(models.Model):
    """One row of the project's imported module list, one per line of the original
    sheet -- the permanent, editable record of what was uploaded (Sr., Module No. and
    Floor Level included, purely for reference). Never deleted by "Confirm & Next"
    (project_import_review): confirming consolidates the current set of these rows
    (grouped by Zone/Segment/Module Type, counts summed) into the project's
    from_import=True ProjectModule rows, and can be re-run any time a correction is
    made here -- the rows themselves stay visible and editable on the Review & Correct
    step for as long as the project exists. A fresh upload either adds to this set or
    replaces it entirely, per the user's choice on the Upload step. Never contributes
    to the project's estimate itself -- only the ProjectModule rows built from it do."""
